...
public class LC850 {
    ...
    public int rectangleArea(int[][] rectangles) {
        ...
        for (int[] rect : rectangles) {
            ...
            eventList.add(new int[]{rect[0], rect[1], rect[3], IN});
            //记录出边 结构是 x -> y1 y2 1/-1
            eventList.add(new int[]{rect[2], rect[1], rect[3], OUT});
            ySet.add(rect[1]);
            ySet.add(rect[3]);
        }
        //将纵坐标离散化，从1开始
        ...
        //按照x轴从小到大排序
        eventList.sort(Comparator.comparingInt(x -> x[0]));
        //开线段树的根节点
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            //分别取出当前边的上下两个坐标
            int left = y2Index.get(event[1]);
            int right = y2Index.get(event[2]);
            //将当前的边的区间更新到线段树上
            segmentTree.update(left, right - 1, event[3], index2y);
            ans += (long) segmentTree.query() * (eventList.get(i + 1)[0] - event[0]);
        }
        return (int) (ans % MOD);
    }
}
